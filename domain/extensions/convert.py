...
from services.airport_name_converter import AirportNameConverter


class HtmlConverter:

    def __init__(self):
        self._airport_name_converter = AirportNameConverter()

    def to_flight(self, html: BeautifulSoup) -> list[Flight]:
        flights = []
        for row in html.find_all('tr'):
            ...
            for airport in route:
                airports.append(self._airport_name_converter.find_match(airport))
    ...
    def to_reserve(self, html: BeautifulSoup) -> list[Reserve]:
        reserves = []
        for row in html.find_all('tr'):
            tds = row.find_all('td')
            ...
            end_datetime = datetime.strptime(
                tds[1].text.replace('\xa0\n\t\t\t\t\t\t\t\t\t\t', ' ').split('[до ')[1][:16],
                '%d.%m.%Y %H:%M')
            reserves.append(Reserve(begin_datetime, end_datetime, info))

        return reserves

    def to_work_event(self, html: BeautifulSoup) -> list[WorkEvent]:
        events = []
        for row in html.find_all('tr'):
            spans = row.find_all('span')
